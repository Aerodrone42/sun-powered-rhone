...
const AgriculturePortfolio = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedImage, setSelectedImage] = useState<number | null>(null);
  const portfolioImages = [{
    src: "/lovable-uploads/25790797-9d84-49fa-90cc-5572f7887337.png",
    alt: "Installation solaire agricole avec tracteur dans les champs",
    title: "Agrivoltaïsme en culture",
    description: "Production d'énergie compatible avec l'activité agricole"
  }, {
    src: "/lovable-uploads/354c5541-3d35-47a0-af91-cb142d7bfa4e.png",
    alt: "Ombrières photovoltaïques pour protection des cultures",
    title: "Ombrières agricoles",
    description: "Protection des cultures et production énergétique"
  }, {
    src: "/lovable-uploads/c94e66b0-3c49-4564-86e2-db422002be17.png",
    alt: "Serre agricole équipée de panneaux solaires",
    title: "Serres photovoltaïques",
    description: "Optimisation énergétique des cultures sous serre"
  }, {
    src: "/lovable-uploads/afae1d8f-4bd9-4ae5-a2f0-5b648d8997f5.png",
    alt: "Installation photovoltaïque sur vignoble",
    title: "Viticulture solaire",
    description: "Protection et production énergétique pour la vigne"
  }];
  ...
  return <>
      <section className="py-16 bg-gradient-to-br from-background via-emerald-500/5 to-green-500/5">
        <div className="container">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
              Installations <span className="bg-gradient-to-r from-emerald-600 via-green-500 to-emerald-400 bg-clip-text text-transparent">agricoles</span>
            </h2>
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">Découvrez des solutions photovoltaïques réalisées pour le secteur agricole</p>
          </div>
          ...
          <div className="relative max-w-6xl mx-auto">
            ...
            <div className="flex justify-center mt-8 space-x-3 overflow-x-auto pb-2">
              {portfolioImages.map((image, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-300 ${currentIndex === index ? 'border-emerald-600 scale-110 shadow-lg' : 'border-transparent hover:border-emerald-400/50 hover:scale-105'}`}>
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {portfolioImages.map((_, index) => <div key={index} className={`h-2 rounded-full transition-all duration-300 ${currentIndex === index ? 'w-8 bg-emerald-600' : 'w-2 bg-gray-300 hover:bg-emerald-400/50'}`} />)}
            </div>
          </div>
        </div>
      </section>
