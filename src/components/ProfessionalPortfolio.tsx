      ...
      <section className="py-16 bg-gradient-to-br from-background via-primary/5 to-secondary/5">
        <div className="container">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">Installations solaires pour entreprises et collectivités</p>
          </div>
          ...
          <div className="relative max-w-6xl mx-auto">
            ...
            <div className="flex justify-center mt-8 space-x-3 overflow-x-auto pb-2">
              {portfolioImages.map((image, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-300 ${currentIndex === index ? 'border-primary scale-110 shadow-lg' : 'border-transparent hover:border-primary/50 hover:scale-105'}`}>
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {portfolioImages.map((_, index) => <div key={index} className={`h-2 rounded-full transition-all duration-300 ${currentIndex === index ? 'w-8 bg-primary' : 'w-2 bg-gray-300 hover:bg-primary/50'}`} />)}
            </div>
          </div>
        </div>
      </section>
